...
// Fetch art data from the database where sellType is equal to 2
$stmt = $pdo->query('SELECT * FROM art WHERE sellType = 2 ORDER BY RAND()');
...
if (isset($_GET['category']) && in_array($_GET['category'], $categories)) {
    ...
    if ($selectedCategory !== 'all') {
        ...
        $stmt = $pdo->prepare('SELECT * FROM art WHERE artType = ? AND sellType = 2 ORDER BY RAND()');
...
if (isset($_GET['search'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM art WHERE (artName LIKE ? OR artType LIKE ? OR frameSize LIKE ? OR artistName LIKE ?) AND sellType = 2 ORDER BY RAND()');
...
<head>
    ...
    <title>Auction - artconnect</title>
...
<body>
    ...
    <div class="title-auc">
        <h1 class="aa">ARTS AVAILABLE IN OUR AUCTION</h1>
    </div>
    <form method="get">
        <div class="search">
